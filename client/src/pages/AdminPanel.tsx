...
export default function AdminPanel() {
  ...
  } = trpc.admin.listTickets.useQuery(undefined, {
  ...
  const completeTicketMutation = trpc.admin.completeTicket.useMutation({
    onSuccess: () => {
  ...
  const handleBanUser = async (userId: number, currentlyBanned: boolean) => {
    await toggleBanMutation.mutateAsync({ userId, isBanned: !currentlyBanned });
  };

  const handleCompleteTicket = async (ticketId: number) => {
    await completeTicketMutation.mutateAsync({ ticketId });
  };
                    ...
                    <Card key={ticket.id} className="border-slate-700 bg-slate-800/50 backdrop-blur-sm p-6">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex flex-col gap-2 ml-4">
                          ...
                          <Button
                            onClick={() => setLocation(`/chat/${ticket.id}`)}
                            variant="outline"
